...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score in range(goal):
        for opponent_score in range(goal):
            if strategy(score, opponent_score) != base:
                return False
    return True
    # END PROBLEM 7
...
def make_averaged(original_function, samples_count=1000):
    ...
    def averaged(*args):
        ...
        for _ in range(samples_count):
            sum_points += original_function(*args)
        return sum_points / samples_count
    return averaged
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    for num_rolls in range(1, 11):
        score = make_averaged(roll_dice, samples_count)(num_rolls, dice)
        if score > best_score:
            best_score = score
